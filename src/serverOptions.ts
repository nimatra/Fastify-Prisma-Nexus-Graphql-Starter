...
export function createAltairConfigurations(): any {
    return {
        ...
        options: { // Pass any additional options for Altair
            theme: 'dark',
            "plugin.list": [
...
export const createMercuriusOptions: any = (server: FastifyInstance) => {
    return {
        ...
        validationRules: [ // Add custom validation rules to the GraphQL server
        ...
        context: async (request: FastifyRequest, reply: FastifyReply): Promise<Context> => { // Set the context for the GraphQL resolver functions
        ...
        persistedQueries: { // Enable persisted queries
        ...
        extensions: [ // Add extensions to the GraphQL server
        ],
        logger: true // Enable Mercurius logger for debugging
    }
};
function validateStringLength(context: { reportError: (arg0: Error, arg1: any[]) => void; }) {
    return {
